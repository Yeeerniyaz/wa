    ...
    return {
        reply_markup: {
            inline_keyboard: [
                ...
                // НОВЫЕ СУПЕРФУНКЦИИ v2.1 и v2.3
                [
                    { text: '🪄 Макросы (Словарь)', callback_data: 'menu_macros' },
                    { text: '⏰ Планировщик', callback_data: 'menu_schedule' },
                ],
                [
                    { text: '💣 Авто-напоминания (Долбежник)', callback_data: 'menu_bomber' },
                ],
                [
...
// Новое меню для Долбежника
const getBomberMenu = () => ({
    reply_markup: {
        inline_keyboard: [
            [{ text: '🔥 Начать атаку (Долбежник)', callback_data: 'ask_bomber_add' }],
            [{ text: '🛑 Принудительно остановить', callback_data: 'ask_bomber_del' }],
            [{ text: '📋 Активные цели', callback_data: 'list_bomber' }],
            [{ text: '🔙 Назад в меню', callback_data: 'back_to_menu' }]
        ]
    }
});
...
tgBot.on('message', async (tgMsg) => {
    ...
    if (state && text && !text.startsWith('/') && !['🚀 Главное меню', '📊 Топ-10 чатов', '📱 Отправить в WA'].includes(text)) {
        ...
        // --- ДОЛБЕЖНИК (АВТО-НАПОМИНАНИЯ) ---
        if (state.action === 'add_bomber_step1') {
            const num = text.replace(/\D/g, '');
            if (!num) return sendToTelegram('❌ Некорректный номер.', cancelKeyboard);
            userStates.set(TG_CHAT_ID, { action: 'add_bomber_step2', data: { num } });
            await sendToTelegram(`💣 Цель *+${num}* захвачена.\n\nТеперь отправь список сообщений, разделяя их знаком \`|\` (вертикальная черта).\n_Пример: Ответь|Ты где?|Аууу_`, cancelKeyboard);
            return;
        }

        if (state.action === 'add_bomber_step2') {
            const texts = text.split('|').map(s => s.trim()).filter(Boolean);
            if (!texts.length) return sendToTelegram('❌ Пустой список. Разделяй фразы знаком `|`. Попробуй еще:', cancelKeyboard);
            userStates.set(TG_CHAT_ID, { action: 'add_bomber_step3', data: { num: state.data.num, texts } });
            await sendToTelegram(`✅ Принято ${texts.length} фраз в обойму.\n\nВведите **интервал отправки в минутах** (например: \`5\` или \`30\`):`, cancelKeyboard);
            return;
        }

        if (state.action === 'add_bomber_step3') {
            const mins = parseInt(text.replace(/\D/g, ''), 10);
            if (isNaN(mins) || mins < 1) return sendToTelegram('❌ Введите корректное число минут (от 1):', cancelKeyboard);
            db.startBomber(state.data.num, state.data.texts, mins);
            await sendToTelegram(`🚀 *Атака запущена!*\nБот будет методично писать абоненту *+${state.data.num}* каждые ${mins} мин, пока тот не ответит.\n\n_Как только придет ответ, цикл автоматически завершится._`);
            userStates.delete(TG_CHAT_ID);
            return;
        }

        if (state.action === 'del_bomber') {
            const num = text.replace(/\D/g, '');
            if (db.stopBomber(num)) {
                await sendToTelegram(`🛑 Атака на +${num} принудительно остановлена.`);
            } else {
                await sendToTelegram(`❓ Для +${num} нет активных циклов.`);
            }
            userStates.delete(TG_CHAT_ID);
            return;
        }
    ...
    if (text === '/start' || text === '/menu' || text === '🚀 Главное меню') {
        ...
        await tgBot.sendMessage(TG_CHAT_ID, '🚀 *ГЛАВНОЕ УПРАВЛЕНИЕ БОТОМ v2.3*\nВыберите категорию:', { parse_mode: 'Markdown', ...getMainMenu() });
        return;
    }
...
tgBot.on('callback_query', async (query) => {
    ...
    if (action === 'menu_bomber') {
        await tgBot.editMessageText('💣 *Авто-напоминания (Долбежник)*\nБот будет присылать абоненту серию сообщений с заданным интервалом, пока абонент не ответит. Идеально для должников.', { chat_id: query.message.chat.id, message_id: query.message.message_id, parse_mode: 'Markdown', ...getBomberMenu() }); return;
    }
    ...
    // --- КНОПКИ ДОЛБЕЖНИКА ---
    if (action === 'ask_bomber_add') {
        userStates.set(TG_CHAT_ID, { action: 'add_bomber_step1' });
        await sendToTelegram('💣 *Запуск*\nВведите номер телефона цели:', cancelKeyboard);
        await tgBot.answerCallbackQuery(query.id); return;
    }
    if (action === 'ask_bomber_del') {
        userStates.set(TG_CHAT_ID, { action: 'del_bomber' });
        await sendToTelegram('🛑 Введите номер для принудительной остановки рассылки:', cancelKeyboard);
        await tgBot.answerCallbackQuery(query.id); return;
    }
    if (action === 'list_bomber') {
        const tasks = db.getBomberTasks();
        if (!Object.keys(tasks).length) return tgBot.answerCallbackQuery(query.id, { text: 'Нет активных целей', show_alert: true });
        const txt = '💣 *Активные цели:*\n\n' + Object.entries(tasks).map(([num, t]) => `👤 +${num}\n⏱ Интервал: ${t.intervalMs/60000} мин\n💬 Фраз: ${t.texts.length}`).join('\n\n');
        await sendToTelegram(txt); await tgBot.answerCallbackQuery(query.id); return;
    }
